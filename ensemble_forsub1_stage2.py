...
def npy2json(model_name, fold_index, cpk, temp=None, w=1):
    ...
    all_names = np.load(os.path.join(resultDir, '%08d_stage2_names.npy' % (cpk)))
    all_results = np.load(os.path.join(resultDir, '%08d_stage2_results.npy' % (cpk)))
    dict_results = {}
    if temp is not None:
        dict_results = temp
...
def ensemble(dict_results, best_t=0.6, num_results=1):
    ...
    for name in tqdm(dict_results.keys()):
        ...
        all_rles.append(rle)
    pd.DataFrame({"ImageId": all_names, "EncodedPixels": all_rles}).to_csv('ensemble_sub_stage2.csv', index=None)
    print(count, count2)
...
ensemble(dict_results, best_t=0.5, num_results=len(results))

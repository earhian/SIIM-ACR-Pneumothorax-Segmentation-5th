...
# log ------------------------------------
def get_split_by_depth(flod, depth_file):
    depth_file = pd.read_csv(depth_file)
    split_lists = []
    for index in range(flod):
        depth_file_single = depth_file[depth_file['fold'] == index]
        depth_file_single_name = depth_file_single['id'].tolist()

        split_lists.append(depth_file_single_name)
    return split_lists

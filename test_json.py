...
def test(model_name, fold_index, checkPoint_start, batch_size):
    ...
    dst_valid = Dataset_pne(test_names, mode='test', transform=transform_test)
    ...
    with torch.no_grad():
        ...
        np.save(os.path.join(resultDir, '%08d_names.npy' % (checkPoint_start)), all_names)
        np.save(os.path.join(resultDir, '%08d_results.npy' % (checkPoint_start)), all_results)
...
if __name__ == '__main__':
    if 1:
        ...
        # model_names = ['seresnext101']
        batch_size = 32
        # checkPoint_starts = [52000, 16000, 28500, 29500, 52000] + [21500]
